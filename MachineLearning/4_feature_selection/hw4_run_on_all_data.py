      ...
      if(d2[j].find('fold') == -1): 
        ...
        for k in range(0, len(d3), 1):
          if(d3[k].find('pos-train') != -1): 
            ...
            os.system('python pearson2.py data trainlabels 10 > feature')
            os.system('python SVM.py feature trainlabels 1 > predictionsvm')
            os.system('python balerror.py labels predictionsvm > errorout') 
            f = open('errorout')
            error = float(f.readline())
            SVMerr += error 
            os.system('python NaiveBayes.py feature trainlabels > predictionnb')
            os.system('python balerror.py labels predictionnb > errorout') 
            f = open('errorout')
            error = float(f.readline())
            nberr += error
            os.system('python NearestMeans.py feature trainlabels > predictionnm')
